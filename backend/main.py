...
@app.get("/")
def root():
    return {"status": "RAG Knowledge Assistant API is running"}
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    parser = PDFParser()
    chunker = Chunker(chunk_size=350, overlap=30)
...
def ask(request: AskRequest):
    try:
        ...
        answer = ask_groq(query=request.query, chunks=chunks)
        sources = [
            {"text": c["text"][:200] + "...", "similarity_score": c["similarity_score"]}
            for c in chunks
        ]
...
def summarize(request: SummarizeRequest):
    try:
        retriever = Retriever(collection_name=request.collection_name)
        chunks = retriever.retrieve(query="main topics overview summary", top_k=request.top_k)
